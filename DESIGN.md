...
- `index.html` — главная: черновики → процесс работы над книгами → первое знакомство → будущая библиотека готовой прозы → блок автора и навигационный подвал.
- `index-previous.html` — сохранённая предыдущая главная для сравнения.
- Три карточки черновиков без фильтров: «Моя слабость, моя боль», «Горячее солнце», «Мелки». В первой установлен авторский арт `images/fin1.png`; в остальных оставлены временные номера. Для замены номера поместите изображение с прозрачным фоном в `.draft-illustration`.
...
Названия, обложки, аннотации и значения прогресса перенесены из существующего проекта. Короткие тексты главной и слоган включают авторские правки в `index.html`. Фрагментов прозы в проекте нет: окна показывают аннотации и явно сообщают о подготовке отрывков. Кнопки «Читать» у новых черновиков пока открывают сообщение о подготовке текста; аннотации других книг к ним не привязаны. Готовые произведения не подменены черновиками.
